...
namespace LZ4s
{
    ...
    public class Lz4sDictionary
    {
        ...
        {
            int size = Lz4sConstants.MaximumCopyFromDistance * 2;
        ...
        {
            uint bucket = Hashing.Murmur3_Mix(key) & (Lz4sConstants.MaximumCopyFromDistance * 2 - 1);

            // On each boundary, swap index parts and clear the current index
            ushort relativePosition = (ushort)(position - _newStart);
            if (relativePosition >= Lz4sConstants.MaximumCopyFromDistance)
            {
            ...
            {
                bucket = (bucket + 1) & (Lz4sConstants.MaximumCopyFromDistance * 2 - 1);
            }
        ...
        public void Matches(uint key, List<long> result)
        {
            ...
            while (_oldPositions[oldBucket] != 0)
            {
                ...
                oldBucket = (oldBucket + 1) & (Lz4sConstants.MaximumCopyFromDistance * 2 - 1);
            }
